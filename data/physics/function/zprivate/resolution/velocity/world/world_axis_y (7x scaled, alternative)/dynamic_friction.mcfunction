# Adjust scales
# (Important): I scale MaxFriction by 100x so it's 70,000x. Then I scale PlanarImpulseSquared by 100x so PlanarImpulse is 70x after being square rooted. Then, the final result is 1,000x.
...
scoreboard players operation #Physics.Maths.Impulse.z Physics /= #Physics.Constants.1000 Physics
    ...
    # IMPULSE IS 7x, CONTACTVELOCITY IS 7,000x
