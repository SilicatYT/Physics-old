...
$execute if score #Physics.InvertValues Physics matches 1 run scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.Object.CrossProductAxis.z$(ObjectAxis).Max Physics
    ...
    # Set the feature
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.ObjectCorner$(StartCorner0).CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_a_$(ObjectAxis) with storage physics:temp data.ObjectEdge.$(ObjectAxis)[0]
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.ObjectCorner$(StartCorner1).CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_a_$(ObjectAxis) with storage physics:temp data.ObjectEdge.$(ObjectAxis)[1]
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.ObjectCorner$(StartCorner2).CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_a_$(ObjectAxis) with storage physics:temp data.ObjectEdge.$(ObjectAxis)[2]
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.ObjectCorner$(StartCorner3).CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_a_$(ObjectAxis) with storage physics:temp data.ObjectEdge.$(ObjectAxis)[3]

# Get the world-geometry block's feature (Edge that's closest to the object)
# (Important): The edge is perpendicular to the axis with the minimum overlap, so it projects onto a single point. Meaning I only have to look at 1 corner point of each of the 4 candidate edges to see which one is farther along the axis.
$execute if score #Physics.InvertValues Physics matches 0 run scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.BlockBase.CrossProductAxis.z$(ObjectAxis).Max Physics
$execute if score #Physics.InvertValues Physics matches 1 run scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.BlockBase.CrossProductAxis.z$(ObjectAxis).Min Physics

    # Set the feature
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.BlockCornerBase0.CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_b_$(ObjectAxis) {Edge:28b,StartCorner:0b,x:"Min",y:"Min",z:"Min"}
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.BlockCornerBase1.CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_b_$(ObjectAxis) {Edge:29b,StartCorner:2b,x:"Max",y:"Min",z:"Min"}
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.BlockCornerBase4.CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_b_$(ObjectAxis) {Edge:30b,StartCorner:4b,x:"Min",y:"Max",z:"Min"}
    $execute if score #Physics.DeepestProjection Physics = #Physics.Projection.BlockCornerBase5.CrossProductAxis.z$(ObjectAxis) Physics run function physics:zprivate/contact_generation/new_contact/world/edge_edge/world_axis_z/get_edge_b_$(ObjectAxis) {Edge:31b,StartCorner:6b,x:"Max",y:"Max",z:"Min"}
...
# Calculate contact data
    ...
    # (Important): The direction is either the object's axis (Already stored) or 1 along that axis (For world blocks). Because I use 1 instead of 1,000, I have to be aware of the scaling.
        # Calculate intermediate results (Dot products)
            ...
            $scoreboard players operation #Physics.Maths.D Physics = #Physics.ThisObject Physics.Object.Axis.$(ObjectAxis).z
        ...
        # Calculate s (On ObjectA's edge)
            # CE - BD
            $execute store result score #Physics.Maths.Value1 Physics run scoreboard players operation #Physics.Maths.s Physics = #Physics.ThisObject Physics.Object.Axis.$(ObjectAxis).z
    ...
    # (Important): Because the block's z axis only has its z component set, the cross product has an z component of 0.
    # (Important): The contact normal scores are also used for accumulation later.
    $execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[0] int 1 run scoreboard players operation #Physics.ContactNormal.x Physics = #Physics.CrossProductAxis.z$(ObjectAxis).x Physics
    $execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[1] int 1 run scoreboard players operation #Physics.ContactNormal.y Physics = #Physics.CrossProductAxis.z$(ObjectAxis).y Physics
