...
$execute store success score #Physics.InvertValues Physics if score #Physics.Projection.OtherObject.CrossProductAxis.y$(OtherAxis).Min Physics < #Physics.Projection.Object.CrossProductAxis.y$(OtherAxis).Min Physics
$scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.Object.CrossProductAxis.y$(OtherAxis).Max Physics
    ...
    $scoreboard players operation #Physics.DeepestProjection Physics -= #Physics.Projection.ObjectCenter.CrossProductAxis.y$(OtherAxis) Physics
    $execute if score #Physics.Projection.OtherObject.CrossProductAxis.y$(OtherAxis).Min Physics >= #Physics.Projection.Object.CrossProductAxis.y$(OtherAxis).Min Physics run function physics:zprivate/contact_generation/new_contact/object/edge_edge/this_axis_y/check_edge_a_max {Axis:"$(OtherAxis)",StartCorner0:0b,StartCorner1:1b,StartCorner2:2b,StartCorner3:3b}
    $execute if score #Physics.Projection.OtherObject.CrossProductAxis.y$(OtherAxis).Min Physics < #Physics.Projection.Object.CrossProductAxis.y$(OtherAxis).Min Physics run function physics:zprivate/contact_generation/new_contact/object/edge_edge/this_axis_y/check_edge_a_min {Axis:"$(OtherAxis)",StartCorner0:0b,StartCorner1:1b,StartCorner2:2b,StartCorner3:3b}

# Get ObjectB's feature (Edge that's closest to ObjectA)
# (Important): The edge is perpendicular to the axis with the minimum overlap, so it projects onto a single point. Meaning I only have to look at 1 corner point of each of the 4 candidate edges to see which one is farther along the axis.
$scoreboard players operation #Physics.DeepestProjection Physics = #Physics.Projection.OtherObject.CrossProductAxis.y$(OtherAxis).Max Physics

    # Set the feature
    # (Important): Because only the min and max projections are scaled down, I need to scale the corner projections down here and turn the DeepestProjection relative again. In addition, to account for rounding errors that are different for positive and negative values (It matters whether I first multiply by -1 and then divide, or the other way around), I turn the min back to the max and just invert which corner matches which corner projection if it tries to get the min projection's corner.
    $scoreboard players operation #Physics.DeepestProjection Physics -= #Physics.Projection.OtherObjectCenter.CrossProductAxis.y$(OtherAxis) Physics
    $execute if score #Physics.Projection.OtherObject.CrossProductAxis.y$(OtherAxis).Min Physics >= #Physics.Projection.Object.CrossProductAxis.y$(OtherAxis).Min Physics run function physics:zprivate/contact_generation/new_contact/object/edge_edge/this_axis_y/check_edge_b_min with data physics:temp data.EdgeData.$(OtherAxis)
    $execute if score #Physics.Projection.OtherObject.CrossProductAxis.y$(OtherAxis).Min Physics < #Physics.Projection.Object.CrossProductAxis.y$(OtherAxis).Min Physics run function physics:zprivate/contact_generation/new_contact/object/edge_edge/this_axis_y/check_edge_b_max with data physics:temp data.EdgeData.$(OtherAxis)
...
# Calculate Penetration Depth, Contact Normal, Contact Point & Separating Velocity
    ...
    # (Important): Edge1 = <Corner on the negative end> + s * <Corner on the positive end> => (u + s * v)    //    Edge2 = <Corner on the negative end> + t * <Corner on the positive end> => (m + t * n)
        # Calculate intermediate results (Dot products)
            ...
            # C = v * n
            execute store result score #Physics.Maths.D Physics run scoreboard players operation #Physics.Maths.C Physics = #Physics.ThisObject Physics.Object.Axis.y.x
            $scoreboard players operation #Physics.Maths.C Physics *= @s Physics.Object.Axis.$(OtherAxis).x

            execute store result score #Physics.Maths.Value7 Physics run scoreboard players operation #Physics.Maths.Value6 Physics = #Physics.ThisObject Physics.Object.Axis.y.y
            ...
            execute store result score #Physics.Maths.Value8 Physics run scoreboard players operation #Physics.Maths.Value6 Physics = #Physics.ThisObject Physics.Object.Axis.y.z
    ...
    $execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[0] int 1 run scoreboard players operation #Physics.ContactNormal.x Physics = #Physics.CrossProductAxis.y$(OtherAxis).x Physics
    $execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[1] int 1 run scoreboard players operation #Physics.ContactNormal.y Physics = #Physics.CrossProductAxis.y$(OtherAxis).y Physics
    $execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[2] int 1 run scoreboard players operation #Physics.ContactNormal.z Physics = #Physics.CrossProductAxis.y$(OtherAxis).z Physics
