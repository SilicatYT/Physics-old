...
# Check if the contact is valid
    ...
    $execute if score #Physics.PenetrationDepth Physics matches ..-1 run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/cross_product_axis_yy/penetration_depth_negative {Edge:$(Edge)b}
...
# Contact Point
    # Calculate intermediate results (Dot products)
        # D = v * (u - m)
        scoreboard players operation #Physics.Maths.D Physics = #Physics.ThisObject Physics.Object.Axis.y.y
    ...
    # Calculate s (On ObjectA's edge)
        # CE - BD
        execute store result score #Physics.Maths.Value1 Physics run scoreboard players operation #Physics.Maths.s Physics = #Physics.ThisObject Physics.Object.Axis.y.y
    ...
    # Calculate t (On ObjectB's edge)
        ...
        scoreboard players operation #Physics.Maths.Value2 Physics /= #Physics.Maths.Value1 Physics
            ...
            # Check if it's invalid
            $execute if score #Physics.Maths.Value2 Physics matches ..-1 run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/cross_product_axis_xy/invalid_contact_point {Edge:$(Edge)b}
            scoreboard players operation #Physics.MaxRange Physics = #Physics.Projection.Block.WorldAxis.y.Max Physics
            scoreboard players operation #Physics.MaxRange Physics -= #Physics.Projection.Block.WorldAxis.y.Min Physics
            $execute if score #Physics.Maths.Value2 Physics > Physics.MaxRange Physics run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/cross_product_axis_xy/invalid_contact_point {Edge:$(Edge)b}
...
execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[0] int 1 run scoreboard players get #Physics.CrossProductAxis.yy.x Physics
execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[2] int 1 run scoreboard players get #Physics.CrossProductAxis.yy.z Physics
execute if score #Physics.InvertValues Physics matches 1 store result storage physics:temp data.NewContact.ContactNormal[0] int -1 run scoreboard players get #Physics.CrossProductAxis.yy.x Physics
execute if score #Physics.InvertValues Physics matches 1 store result storage physics:temp data.NewContact.ContactNormal[2] int -1 run scoreboard players get #Physics.CrossProductAxis.yy.z Physics
...
# Separating Velocity
    ...
    scoreboard players operation #Physics.PointVelocity.z Physics *= #Physics.CrossProductAxis.yy.z Physics
