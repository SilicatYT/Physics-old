...
# Check if the contact is valid
    ...
    $execute if score #Physics.PenetrationDepth Physics matches ..-1 run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/cross_product_axis_xz/penetration_depth_negative {Edge:$(Edge)b}
...
# Contact Point
    # Calculate intermediate results (Dot products)
        # D = v * (u - m)
        scoreboard players operation #Physics.Maths.D Physics = #Physics.ThisObject Physics.Object.Axis.z.x
        ...
        scoreboard players operation #Physics.Maths.Value3 Physics *= #Physics.ThisObject Physics.Object.Axis.z.z
        scoreboard players operation #Physics.Maths.D Physics += #Physics.Maths.Value3 Physics

    # Calculate s (On ObjectA's edge)
        # CE - BD
        execute store result score #Physics.Maths.Value2 Physics run scoreboard players operation #Physics.Maths.s Physics = #Physics.ThisObject Physics.Object.Axis.z.x
    ...
    # Calculate t (On ObjectB's edge)
        ...
        scoreboard players operation #Physics.Maths.Value1 Physics /= #Physics.Maths.Value2 Physics
            ...
            # Check if it's invalid
            $execute if score #Physics.Maths.Value1 Physics matches ..-1 run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/cross_product_axis_xz/invalid_contact_point {Edge:$(Edge)b}
            ...
            $execute if score #Physics.Maths.Value1 Physics > Physics.MaxRange Physics run return run function physics:zprivate/contact_generation/accumulate/world/touching/update_contact/cross_product_axis_xz/invalid_contact_point {Edge:$(Edge)b}
...
execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[1] int 1 run scoreboard players get #Physics.CrossProductAxis.xz.y Physics
execute if score #Physics.InvertValues Physics matches 0 store result storage physics:temp data.NewContact.ContactNormal[2] int 1 run scoreboard players get #Physics.CrossProductAxis.xz.z Physics
execute if score #Physics.InvertValues Physics matches 1 store result storage physics:temp data.NewContact.ContactNormal[1] int -1 run scoreboard players get #Physics.CrossProductAxis.xz.y Physics
execute if score #Physics.InvertValues Physics matches 1 store result storage physics:temp data.NewContact.ContactNormal[2] int -1 run scoreboard players get #Physics.CrossProductAxis.xz.z Physics
...
# Separating Velocity
    ...
    scoreboard players operation #Physics.VelocityFromAcceleration.y Physics *= #Physics.CrossProductAxis.xz.y Physics
    ...
    scoreboard players operation #Physics.SubtractVector.y Physics *= #Physics.CrossProductAxis.xz.y Physics
    scoreboard players operation #Physics.SubtractVector.z Physics *= #Physics.CrossProductAxis.xz.z Physics
    ...
    scoreboard players operation #Physics.PointVelocity.y Physics *= #Physics.CrossProductAxis.xz.y Physics
